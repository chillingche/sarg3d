...
def trace_ray(rayO, rayD):
    ...
    l = [
        intersect(M + N * .0001, toL, obj_sh) for k, obj_sh in enumerate(scene)
        if k != obj_idx
    ]
    ...
    col_ray += obj.get('specular_c', specular_c) * max(
        np.dot(N, normalize(toL + toO)), 0)**specular_k * color_light
